...
^^^^^^^^^^^^^^
    ...
    * ``("dealStat","int","PoolCollectedPeriod")`` -> number of pool colllection periods.
    ...
    .. versionadded:: 0.51.6
    * ``("activeBondNum",)`` -> number of active bonds
...
available formula options:
  ...
  * ``("poolBalance","A")`` -> pool balance of pool "A"
  * ``("bondBalance",)`` -> total bond balance
  * ``("bondBalance","A")`` -> bond balance of bond "A"
  * ``("originalPoolBalance",)`` -> original pool balance
  * ``("originalBondBalance",)`` -> original total bond balance
...
examples: 
    ...
    * ``{"balCapAmt": 500}`` ,  pay up to 500.
    * ``{"formula": <formula> }``, pay up the :ref:`Formula`
    * ``{"DS": <formula> }``, pay up the :ref:`Formula`
    * ``{"ds": <formula> }``, pay up the :ref:`Formula`
    * ``("balPct", 0.6)`` , pay up to 60% of due amount
    * ``("balCapAmt", 500)`` ,  pay up to 500
    * ``("formula", <formula> )``, pay up the :ref:`Formula` 
